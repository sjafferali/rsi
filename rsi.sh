...
then
	...
	print_sub "https://"$(curl curlmyip.de)":8443/login_up.php3?login_name=admin&passwd="$(if [[ $(rpm -q psa | awk -F"-" '{print$2}' | sed 's/\.//g') -le "1019" ]] ; then cat /etc/psa/.psa.shadow ; else /usr/local/psa/bin/admin --show-password ; fi)
